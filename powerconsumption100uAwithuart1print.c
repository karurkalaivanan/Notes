...
#define RTC_WAKEUP_SECONDS    60U
#define RTC_WAKEUP_COUNTER    (RTC_WAKEUP_SECONDS - 1U)
...
/* USER CODE BEGIN PFP */
static void MX_RTC_Custom_Init(void);
static void Prepare_GPIO_For_Stop(void);
static void Restore_GPIO_After_Stop(void);
...
int main(void)
{
  ...
  uint8_t tx_msg[] = {
      ...
      0x03, 0x03, 0x81, 0x0A, 0x00, 0xAF, 0x16
  };
  uint8_t wake_msg[] = "WAKE\r\n";
  HAL_Delay(10000);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    SystemClock_Config();
    HAL_ResumeTick();
    Restore_GPIO_After_Stop();
    HAL_UART_Transmit(&huart1, wake_msg, sizeof(wake_msg) - 1U, HAL_MAX_DELAY);
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc_local)
{
  (void)hrtc_local;
  rtc_alarm_flag = 1U;
}
